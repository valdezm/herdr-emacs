...
#
# NB: auto-open is a shell wrapper, not a herdr plugin — herdr has no session-level plugin
# event (no session.created), so there's no plugin hook for "a new session started".
#
