...
this is a fork of [ogulcancelik/herdr](https://github.com/ogulcancelik/herdr) that adds three things on top of upstream:

1. an **emacs-style keymap** (`C-x` prefix + emacs window commands) — [`config.toml`](./config.toml),
2. the **herdr agent skill installable into Claude Code with one command** (Claude Code plugin), and
3. a shipped **keybindings cheat sheet** ([`keys.md`](./keys.md)) you can open in its own pane.
...
```bash
# 1. emacs keymap → ~/.config/herdr/config.toml (backs up any existing config, then reloads)
./install.sh

# 2. the Claude Code skill — one command, no file copying
...
| `C-x ←` / `C-x →` | previous / next agent |
| `C-x p` / `C-x n` | previous / next tab |
| `C-x k` | close tab (kill-buffer) |
...
the [`herdr` skill](./skills/herdr/SKILL.md) lets a Claude Code agent **drive herdr from inside a pane** — split panes, spawn sibling agents, tail logs, wait for output — over herdr's local unix socket. it installs as a Claude Code plugin (the `install` step above) and activates automatically when a Claude session runs inside a herdr pane (`HERDR_ENV=1`; check with `echo $HERDR_ENV`).
...
**Automatic (recommended) — native herdr plugin:**

```bash
herdr plugin install valdezm/herdr-emacs     # or, from a local clone: ./install.sh --plugin
```

This installs a herdr plugin whose **`session.created`** event hook opens the cheat-sheet **info** workspace on every new session — no shell-rc edit. (The event command inherits `HERDR_SOCKET_PATH`, so it targets the new session automatically.)

**Alternative — shell wrapper:** if you'd rather not install a herdr plugin, `./install.sh --hook` adds a `herdr()` wrapper that runs `ensure-info.sh` on each launch/attach instead. (Both paths call the same idempotent script, so having both installed is harmless — whichever fires first wins.)

Either way the workspace is **split into panes**, one section group per pane — keymap (left), file-viewer keys (top-right), agent recipes (bottom-right) — so the sections don't all share one scroll. `ensure-info.sh` is idempotent (creates the `info` workspace only if absent, never duplicates); `keys-view.sh "<section>" ...` renders only the named `##` sections of `keys.md`.
