...
> Why a shell wrapper and not a herdr plugin? herdr's plugin event hooks (`[[events]]`) only cover workspace/tab/pane/worktree events — there is **no session-level event** (no `session.created`), so a plugin can't hook "a new session started". The `herdr()` wrapper fires on launch/attach, which is exactly the needed moment.

The workspace is **split into panes**, one section group per pane — keymap (left), file-viewer keys (top-right), agent recipes (bottom-right) — so the sections don't all share one scroll. (`keys-view.sh "<section>" ...` renders only the named `##` sections of `keys.md`.)
